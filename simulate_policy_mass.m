%% SIMULATE LAPSE
lapse          = rand(N,M);
lapse(:,1)     = lapse(:,1) <= 0.40;
lapse(:,2:end) = lapse(:,2:end) <= probLapse;
...
for ii=1:N
    ...
    if (lapseT(ii) <= deathT(ii)) && (lapseT(ii) <= size(F,2))
        ...
        revenue(ii,lapseT(ii))   = revenue(ii,lapseT(ii)) + benefit_cost;
    ...
    elseif (deathT(ii) < lapseT(ii)) && (deathT(ii) <= size(F,2))
        ...
        revenue(ii,deathT(ii))   = revenue(ii,deathT(ii)) + benefit_cost;
    ...
    % NEITHER
        ...
        revenue(ii,M)   = revenue(ii,M) + benefit_cost;
